...
### Consumption                      🚧 Week 7 (dbt done; dashboard manual)
- Three pre-aggregated marts (`mart_repo_health`,
  `mart_language_contributor_trends`, `mart_pr_velocity`) — KB-scale, one
  per dashboard question; scoped to the tracked repos. ✅
- dbt `exposure` `contributor_health_dashboard` in
  `models/marts/_exposures.yml`, depends on the three marts. ✅
- Looker Studio dashboard + public link — assembled in the browser
  (panel specs in `docs/week-7.md`). ⏳
